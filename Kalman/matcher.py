...
class Matcher:
    ...
    def match(cls, state_list, measure_list):  # 两个list中单个元素数据结构都是[c_x, c_y, w, h].T
        ...
        for idx_sta, state in enumerate(state_list):
            ...
            for idx_mea, measure in enumerate(measure_list):
                ...
                score = cls.cal_det(state, measure)
    ...
    @classmethod
    def cal_det(cls, state, measure):
        """
        计算状态和观测（截矩+斜率）的差值
        :param state:ndarray [c0, c1, c2, c3, det_c0, det_c1, det_c2, det_c3].T
        :param measure:ndarray [c0, c1, c2, c3, det_c0, det_c1, det_c2, det_c3].T
        :return:
        """
        s_c2, s_c3 = state[2], state[3]
        m_c2, m_c3 = measure[2], measure[3]
        #print(str(abs(s_c2+s_c3-m_c2-m_c3)))
        return 100-0.0001*abs(s_c3-m_c3)-0.1*abs(s_c2-m_c2)

if __name__ == '__main__':
    state_list = [np.array([10, 10, 3, 2, 5, 22, 4, 2]).T, np.array([30, 30, 5, 5, 12, 8, 6, 4]).T]
    # state_list = []
    measure_list = [np.array([12, 12, 2, 3, 7, 11, 6, 5]).T, np.array([28, 28, 5, 5, 5, 7, 9, 10]).T]
